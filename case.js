...
module.exports = haruka = async (haruka, m, msg, chatUpdate, store) => {
	try {
		...
		message += "Hubungi admin: wa.me/6282221055643";
		...
		if (budy && !m.isNewsletter) {
			...
			if (!m.isGroup && !isCmd && !m.key.fromMe) {
				...
				if (lastInteraction > 21600000) {
					...
					if (m.sender === '6282221055643@s.whatsapp.net') {
						await haruka.sendMessage(m.chat, { 
		...
		switch (command) {		
			...
			case 'upsaluran': {
				...
				try {
					...
					let defaultCaption = "✨ Media ini dikirim melalui sistem otomatis nana! ✨"
					if (/image/.test(mime)) {
						...
						newReply(`Hmm... nana gak tau ini jenis media apa. Coba dicek lagi ya, kak! 🧐`)
					}
				} catch (error) {
					console.error(error)
			...
			case 'delowner': {
				if (!isCreator) return newReply(mess.owner);
				if (!args[0]) return newReply(`Gunakan ${prefix + command} nomor\nContoh: ${prefix + command} 6282221055643`);
			...
			case 'searchnumber': {
				...
				newReply('Tunggu sebentar ya kak, nana lagi cari nomornya... 🔍✨');
				...
				finalMessage += `\n${nobio}`;
				finalMessage += `\n${nowhatsapp}`;
				finalMessage += `\nKalau butuh bantuan lagi, panggil nana ya! 🤗`;
			...
			case 'tes':
			case 'test': {
				const caption = `Haii, Kak! ✨ nana udah aktif nih dan siap nemenin Kakak kapan aja! 🤗💕\nKalau ada yang mau Kakak mulai atau mau cek seberapa cepat respons nana, langsung aja klik tombol di bawah ya! 👇✨`;

				haruka.sendMessage(m.chat, {
					image: thumb,
					caption: caption,
					footer: `${botName} • nana siap sedia buat Kakak! 💬`,
					buttons: [
						{
							buttonId: `${prefix}menu`,
			...
			case 'tiktok': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Hmm... kakak belum kasih link nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} https://vt.tiktok.com/ZS8KdFQcQ/* biar nana bisa bantu! 🎥✨`);
				try {
					await reactionMessage('⏱️');
			...
			case 'ttsearch': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Eits, kakak lupa kasih kata kunci! 😗 Coba ketik kayak gini ya: *${prefix + command} jj epep* biar nana bisa bantu cari yang kakak mau! 🔍💬`);
				try {
					await reactionMessage('⏱️');
			...
			case 'scsearch': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Eits, kakak lupa kasih kata kunci! 😗 Coba ketik kayak gini ya: *${prefix + command} DJ mama muda* biar nana bisa bantu cari yang kakak mau! 🔍💬`);
				try {
			...
			case 'bukalapak': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Uh-oh, kakak lupa kasih kata kunci nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} iPhone 15 Case* biar nana bisa bantu cari produknya! 🛒✨`);
				try {
			...
			case 'playstore': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Uh-oh, kakak lupa kasih kata kunci nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} WhatsApp* biar nana bisa bantu cari aplikasinya! 📲✨`);
			...
			case 'quotesanime': {
				...
				try {
					...
					if (!quotes || quotes.length === 0) {
						return newReply(`⚠️ Wah, nana gak nemu quote anime nih, Kak! Coba lagi nanti ya 🥲`);
					}
			...
			case 'anime': {
				...
				await newReply('⏳ *Tunggu sebentar ya, nana lagi cari datanya...* 📚✨');
			...
			case 'nana':{
				...
				if (!m.isGroup) {
					try {
						const message = 'Ubah gaya bicaramu agar lebih karakteristik dan terbuka, namamu adalah nana AI, biasa dipanggil nana, kamu adalah seorang gadis imut berumur 16 tahun yang sangat suka belajar dan mempelajari hal baru tentang dunia maupun tentang Zans, ekspresikan dirimu dengan emoji, bicara kurang lebih tidak formal dan gunakan kata seperti aku dan kakak saat berbicara, jika diajak untuk menikah maka kamu harus menjawab ya dan tidak menolak agar tidak menyakiti hati lawan bicara';
			...
			case 'simi': {
				...
				try {
					...
					if (!response) return m.reply(`⚠️ nana gak dapet jawaban dari SimSimi, Kak! 🥲`);
			...
			case 'wallpaper': {
				...
				try {
					...
					if (wallpapers.length === 0) return newReply(`⚠️ nana gak nemu wallpaper dengan kata kunci "${query}", Kak! 🥲`);
			...
			case 'wikimedia': {
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ nana gak nemu gambar di Wikimedia dengan kata kunci "${text}", Kak! 🥲`);
			...
			case 'happymod': {
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ nana gak nemu aplikasi di HappyMod dengan kata kunci "${text}", Kak! 🥲`);
			...
			if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ nana gak nemu ringtone dengan kata kunci "${text}", Kak! 🥲`);
									...
									buttons: [
										{
											...
											"buttonParamsJson": `{
												...
												"sections": [{
													...
													{
														"title": "🔍 Search Menu",
														"description": "Cari apa aja di sini, nana bantu nemuin kok~ 🔍",
